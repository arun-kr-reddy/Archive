...
- **Fast != Efficient:**
  - just because program runs faster, doesn't mean its using the hardware efficiently  
...
- **Why Parallelism?:**
  - single-threaded performance used to double every ~18 months
  - smaller transistors enabled higher clock freq and more transistors in same area
  - `power ∝ (freq * voltage^2)` but `volatage ∝ freq`, so dynamic power cubes as freq increases
  - extra transistors were used to accelerate single instruction stream ILP (*example:* better OoO & branch prediction)
  - but ILP has diminishing returns beyond 4 IPC (not enough parallelism in code)
  - nowadays extra transistors instead used to add more cores & specialized units (*example:* SIMD, NPUs)
- ***Example:* Modern Processor:**
  - 4 core, 3-way superscalar, 8-wide SIMD
  - ![](./Media/Modern_Processor.png)
- **Coherant Execution:**
  - (most) data elements following same control flow path
  - necessary for efficient SIMD, but not for multi-core
...
- **Superscalar Execution:**
  - exploit ILP within instruction stream
  - processor during execution finds independent instructions and run them on different execution units (same core)
...
- **Cache:**
  ...
  - cache lines also help with spatial locality
  - cold miss, capacity miss, conflict miss
  - mapping of cache line to cache region is fixed
  - caches reduce length of memory stalls, especially for recently accessed data
- **SIMD:**
  - multiple ALUs controlled by same instruction (within a core)
  - amortizes control costs of managing instruction streams across many ALUs
  - intrinsics == macros
- **Multi-Core:**
  - executing completely different instruction streams on each core
  - software creates threads to expose this parallelism to hardware
- **Multi-Threading:**
  - interleave processing of multiple threads on same core hides stalls
- **POD Data Type:**