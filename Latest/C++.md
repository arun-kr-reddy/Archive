...
- [Classic STL](https://www.youtube.com/watch?v=tXUXl_RzkAk)
- [STL Containers](https://www.youtube.com/watch?v=ZMUKa2kWtTk)
...
- **Comparison:**
  - first size compared, then elements compared one-by-one
  - all containers support `==` & `!=`
  - only sequence & ordered containers support `<`, `<=`, `>` & `>=`, result determined by first mismatched lane
- **Range-Based Loop:**
  ```cpp
...
- **Special Elements:**
    ```cpp
    T val = vec.front(); // first element
    T val = vec.back();  // last element
    ```
- **Index Access:**
  ```cpp
  T val = vec[i];    // bounds not checked
  T val = vec.at(i); // bounds checked
  ```
- **Insert vs Emplace:**
...
- **List Special Functions:** optimized for pointer manipulation
  - 
    ...
    lst.splice(another_lst_itr, another_lst, start_itr, end_itr);
                            // move elements in iterator range
    ```
  - `forward_list` only stores pointer to next element, so only next can be modified
    ```cpp
    fwd_lst.erase_after(itr);
    fwd_lst.insert_after(itr, val);
    splice_after(another_lst_itr, another_lst, start_itr, end_itr);
    ```
- **`vector<bool>`:**
  - space-efficient specialization where each element occupies single bit
  - different lanes within same byte (smallest addressable unit) shouldn't be concurrently modified
...
- **Insert:**
  ...
  pos_itr = set.insert(hint_itr, val); // pass hint for `O(1)` insertion
  ```
...
- **Misc:**
  - `[]` operator creates new element (with default-constructed value) if key already not present
    ...
    for (auto &key : container)
      mp[key]++; // key count
    ```
  - `.erase(key)` will erase all element with `key` in multi-*, instead use `.erase(itr)`
  - create associative containers using sequence
    ```cpp
...
- **Bucket Interface:**
  ...
  float lf = ust.loadfactor();    // average elements per bucket
...
- **Hierarchy of Requirements:** lower category supports all operations of ones above it (output is optional)  
  *example:* bi-directional supports read & multi-pass (forward's properties)  
  ![](./Media/Iterators_Hierarchy.png)
...
- **Iterator Invalidation:**  
  ![](./Media/Iterator_Invalidation.png)
- **Misc:**
